...
class SelfAttention(nn.Module):
    """自注意力機制 - 可選擇性關閉以應對條紋問題"""
    def __init__(self, in_dim, activation=F.relu, with_attn=True):
...
def train_cyclegan():
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=2)  # 降低 workers 避免問題
    ...
    dcp_loss = DCPLoss(weight=lambda_dcp, w_min=0.1, w_max=0.5)  # 保守的權重範圍
    ...
    for epoch in range(num_epochs):
        ...
        for i, (real_A, real_B) in enumerate(dataloader):
            ...
            # 強霧 Gamma 校正 (1.1~1.3) - 基於霧濃度動態調整
            with torch.no_grad():
                haze_density_for_gamma = haze_estimator.estimate_haze_density(real_A)
                # 強霧區域 (>0.6) 應用 gamma 校正提升亮度
                gamma_values = torch.where(
                    haze_density_for_gamma > 0.6,
                    1.1 + 0.2 * torch.clamp((haze_density_for_gamma - 0.6) / 0.4, 0, 1),  # 1.1~1.3
                    torch.ones_like(haze_density_for_gamma)  # 無校正
                )
