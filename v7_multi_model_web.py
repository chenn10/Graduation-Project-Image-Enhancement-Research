...
class SelfAttention(nn.Module):
    """CBAM 替代版 SelfAttention - 在內部使用 CBAM，但保留類名以相容現有呼叫。
    支援 with_attn 開關以關閉 attention。"""
    def __init__(self, in_dim, activation=F.relu, with_attn=False, reduction=16, kernel_size=7):
...
def get_all_v7_models():
    ...
    改進：同時搜尋 `checkpoints` 與常見錯誤拼寫 `cheakpoint`，以便短期容錯。
    """
    patterns = [
        'checkpoints/cyclegan_v7_enhanced_no_attn_epoch_*.pth',
        'checkpoints/cyclegan_v7_enhanced_epoch_*.pth',
        'cheakpoint/cyclegan_v7_enhanced_no_attn_epoch_*.pth',
        'cheakpoint/cyclegan_v7_enhanced_epoch_*.pth',
    ]

    v7_models = []
    for p in patterns:
        v7_models.extend(glob.glob(p))

    if not v7_models:
        return []

    # 按 epoch 數字排序，容錯處理檔名解析失敗時回傳 0
    def epoch_from_path(path):
        try:
            return int(os.path.basename(path).split('_epoch_')[1].split('.')[0])
        except Exception:
            return 0

    v7_models.sort(key=epoch_from_path)
    return v7_models
...
def load_v7_model(model_path=None):
    ...
    try:
        ...
        # 載入權重（使用 strict=False 容忍 attention module 的參數差異）
        ...
        if 'generator_AB' in checkpoint:
            ...
            try:
                res = generator.load_state_dict(state, strict=False)
                # res is a NamedTuple with missing_keys and unexpected_keys in newer PyTorch
                missing = getattr(res, 'missing_keys', None)
                unexpected = getattr(res, 'unexpected_keys', None)
                if missing:
                    print(f"⚠️ 載入時缺少參數 ({len(missing)}): {missing[:8]}{'...' if len(missing)>8 else ''}")
                if unexpected:
                    print(f"⚠️ 載入時多餘參數 ({len(unexpected)}): {unexpected[:8]}{'...' if len(unexpected)>8 else ''}")

                current_model_path = model_path
                print(f"✅ 成功載入 v7 模型（部分參數若不匹配會被忽略）: {model_path}")
            except Exception as e:
                print(f"❌ 嘗試以寬鬆模式載入 generator_AB 失敗: {e}")
                return False, str(e)
        else:
